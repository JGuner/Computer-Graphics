...
void main() {
  ...
  vec4 vertCoord = vec4(vertTexCoord.x, vertTexCoord.y, 0.0, 1);
  vec4 center = vec4(0.5, 0.5, 0, 1);
  float centerDist = distance(vertCoord, center);
  offset = (sin(centerDist * 50) + 1) / 2;
  vec4 shiftVect = (50 * offset) * vec4(normal, 0);
  gl_Position = transform * (vertex + shiftVect);
}
